...
export async function startTestDb(): Promise<DynamoDBClient> {
  ...
  if (!process.env.TABLE_NAME && !process.env.DYNAMODB_CORE_TABLE_NAME) {
    process.env.TABLE_NAME = "formulary-core-test";
    process.env.DYNAMODB_CORE_TABLE_NAME = "formulary-core-test";
  }
  if (!process.env.BASE_TABLE_NAME && !process.env.DYNAMODB_BASE_TABLE_NAME) {
    process.env.BASE_TABLE_NAME = "formulary-base-test";
    process.env.DYNAMODB_BASE_TABLE_NAME = "formulary-base-test";
  }
