...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  ...
  const profile: SandboxTenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Formulary Demo' : tenantId,
    displayNameAr: tenantId === 'tnt_demo' ? 'تجربة الصيدلية' : tenantId,
  };
...
export function generateApiKeySecret(prefix = 'formulary_demo'): string {
